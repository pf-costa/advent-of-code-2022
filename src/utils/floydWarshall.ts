...
const findEdge = (graph: Graph, from: string, to: string) => {
...
export default function floydWarshall(graph: Graph) {
  ...
  const vertexIndices = Object.keys(graph).reduce(
    (acc, key, i) => ({ ...acc, [key]: i }),
  ...
  return { distances, nextVertices, vertexIndices };
}
